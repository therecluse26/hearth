...
use hearth::core::{SystemClock, TenantId};
use hearth::storage::{EmbeddedStorageEngine, StorageConfig, StorageEngine};

/// Build a `CreateAuditEvent` with a synthetic sequence marker so tests can
/// assert ordering.
fn make_event(tenant_id: TenantId, seq: u32) -> CreateAuditEvent {
    CreateAuditEvent {
        tenant_id,
...
fn simulation_audit_crash_mid_batch_discards_partial_event() {
    ...
    // Phase 1: two successful appends.
    {
        ...
        audit
            .append(&make_event(tenant.clone(), 1))
            .expect("append 1");
        audit
            .append(&make_event(tenant.clone(), 2))
            .expect("append 2");
    }
    ...
    // Phase 3: reopen and verify invariant.
    {
        let (_storage, audit) = open_audit(dir.path());
        let events = audit
            .query(&AuditQuery::for_tenant(tenant.clone()))
        ...
        let valid = audit
            .verify_integrity(&tenant, None, None)
...
fn simulation_audit_crash_corrupt_crc_rolls_back_last_event() {
    ...
    // Phase 1: three successful appends.
    {
        ...
        for i in 1..=3 {
            audit
                .append(&make_event(tenant.clone(), i))
                .expect("append");
        }
    }
...
fn simulation_audit_integrity_under_sustained_load() {
    ...
    for thread_id in 0..THREADS {
        ...
        let handle = thread::spawn(move || {
            for i in 0..EVENTS_PER_THREAD {
                ...
                audit
                    .append(&make_event(tenant.clone(), marker))
    ...
    // engine tie-breaks duplicates by sequence within a tenant).
