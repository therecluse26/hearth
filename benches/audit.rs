...
//!
//! The benchmark pre-populates a tenant with 100,000 audit events
...
fn setup_audit() -> (tempfile::TempDir, EmbeddedAuditEngine, TenantId) {
...
fn bench_audit_query_by_actor(c: &mut Criterion) {
    let (_dir, engine, tenant_id) = setup_audit();
...
/// Benchmarks an unbounded (tenant-only) audit query — worst case.
