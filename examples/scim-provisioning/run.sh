#!/usr/bin/env bash
# One-command walkthrough of Hearth's SCIM 2.0 provisioning API.
#
#   ./run.sh
#
# What happens:
#   1. `cargo build` (release profile) — may take a while on first run.
#   2. Wipes any prior demo data under `./data/scim-provisioning/`.
#   3. Starts Hearth in the background (--dev, HTTP 8420).
#   4. Runs `demo.mjs` which drives the full SCIM surface end-to-end.
#   5. Kills Hearth and exits with the demo's status code.

set -euo pipefail

HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$HERE/../.." && pwd)"
DATA_DIR="$HERE/data/scim-provisioning"
...
echo "▸ starting Hearth (HTTP 8422)"
(
  cd "$HERE"
...
wait_for "http://127.0.0.1:8422/health"
