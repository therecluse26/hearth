...
mod tests {
    ...
    // ===== Phase C: Simulation stubs =====

    /// Tier transitions preserve all data under concurrent read/write load.
    ///
    /// Multiple threads concurrently promote, read, and invalidate entries.
    /// After all threads finish, every entry that was promoted (and not
    /// invalidated) must be readable, proving the `ArcSwap` clone-mutate-swap
    /// pattern is safe under contention.
    #[test]
    fn simulation_tier_transitions_concurrent() {
        use std::sync::Arc;

        let tier = Arc::new(HotTier::new(TieredConfig {
            hot_tier_capacity: 200,
            eviction_batch_size: 10,
        }));
        let tenant = TenantId::generate();

        // Phase 1: Pre-populate 50 entries
        for i in 0u32..50 {
            let key = format!("conc-{i:04}");
            tier.promote(&tenant, key.as_bytes(), b"initial");
        }

        // Phase 2: Concurrent operations from multiple threads
        let mut handles = Vec::new();

        // Writer threads: promote new entries
        for batch in 0u32..4 {
            let tier = Arc::clone(&tier);
            let t = tenant.clone();
            handles.push(std::thread::spawn(move || {
                for i in 0u32..25 {
                    let key = format!("writer-{batch}-{i:04}");
                    tier.promote(&t, key.as_bytes(), b"written");
                }
            }));
        }

        // Reader threads: read existing entries
        for _ in 0..4 {
            let tier = Arc::clone(&tier);
            let t = tenant.clone();
            handles.push(std::thread::spawn(move || {
                for i in 0u32..50 {
                    let key = format!("conc-{i:04}");
                    // Read may return None if evicted under pressure, that's ok
                    let _ = tier.get(&t, key.as_bytes());
                }
            }));
        }

        // Invalidation thread: remove some entries
        {
            let tier = Arc::clone(&tier);
            let t = tenant.clone();
            handles.push(std::thread::spawn(move || {
                for i in (0u32..50).step_by(5) {
                    let key = format!("conc-{i:04}");
                    tier.invalidate(&t, key.as_bytes());
                }
            }));
        }
        ...
        // Phase 3: Verify structural integrity — no panics, no data corruption.
        // Every entry that exists must have a valid value.
        let len = tier.len();
        assert!(
            len > 0,
            "tier should still have entries after concurrent ops"
        );
        assert!(len <= 200, "tier should not exceed capacity ({len} > 200)");

        // Verify a sampling of entries that were NOT invalidated
        for i in (1u32..50).step_by(5) {
            let key = format!("conc-{i:04}");
            // These may or may not be evicted, but if present, the value must be correct
            if let Some(val) = tier.get(&tenant, key.as_bytes()) {
                assert_eq!(
                    val, b"initial",
                    "non-invalidated entry must have correct value"
                );
            }
        }
    }
    ...
    /// an empty tier on restart. Verify that recovery from WAL+SST
    /// correctly re-populates the hot tier on first access.
    #[test]
    fn simulation_crash_during_promotion() {
        ...
        // Phase 1: Write data and interact with the hot tier
        {
            ...
            // Write data (goes to WAL + memtable, may flush to SST)
            for i in 0u32..10 {
            ...
            // Read to promote entries into hot tier
            for i in 0u32..10 {
                let key = format!("hot-{i:04}");
        ...
        // Drop = simulates crash. Hot tier is lost (in-memory only).

        // Phase 2: Re-open — hot tier is empty, data lives in WAL+SST
        {
            let config = StorageConfig {
                data_dir: dir.path().to_path_buf(),
                wal_config: crate::storage::wal::WalConfig {
            ...
            // All data must be recoverable from WAL+SST (cold path)
            for i in 0u32..10 {
                let key = format!("hot-{i:04}");
                let val = engine.get(&tenant, key.as_bytes()).expect("get");
                assert_eq!(
                    ...
                    "key {key} must be recoverable from WAL+SST after crash"
                );
            }

            // After first read, entries should be promoted back to hot tier
            // (second read should be a hot-tier hit)
            for i in 0u32..10 {
                let key = format!("hot-{i:04}");
                let val = engine.get(&tenant, key.as_bytes()).expect("get");
                assert_eq!(val, Some(b"hot-value".to_vec()));
